...
BEGIN
    ...
    RETURN 0
END
-- exec InsertEmployee 'Common', 'Martin', '1234567890', 0
...
BEGIN
    ...
    BEGIN
        ...
        RETURN -2
    END
    IF NOT EXISTS(SELECT EmployeeID FROM Employees WHERE EmployeeID = @Employee)
    BEGIN
...
-- ticket
CREATE PROCEDURE CreateTicket (@SeanceId tinyint, @TypeName tinyint, @CashboxID tinyint, @RowNumber tinyint, @SeatNumber tinyint, @Cost smallint)
    AS
BEGIN
    IF NOT EXISTS(SELECT SeanceId FROM Seances WHERE SeanceId = @SeanceId)
    BEGIN
        ...
        RETURN -2
    END
    IF NOT EXISTS(SELECT TypeName FROM TicketTypes WHERE TypeName = @TicketType)
    BEGIN
