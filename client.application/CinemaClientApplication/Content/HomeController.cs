...
namespace CinemaClientApplication.Controllers
{
    public class HomeController : Controller
    {
        ...
        {
            int film = int.Parse(form["FilmID"]);
            string seance = form["Seance"];
            string tickets = form["Tickets"];
            requestDAO.RemoveTickets(seance, tickets);
            return Redirect("~");
        }
        [HttpGet]
        public async Task<PartialViewResult> _SeancesSearch(int film)
        {
